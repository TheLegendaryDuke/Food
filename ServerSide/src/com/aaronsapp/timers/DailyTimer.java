 ...
 */
@BackendlessTimer("{'startDate':1486702800000,'frequency':{'schedule':'custom','repeat':{'every':60}},'timername':'daily'}")
...
//    log.debug(String.valueOf(res.size()));
		...
		res.clear();
		res = new ArrayList(Backendless.Persistence.of("foodOffers").find(q).getCurrentPage());
		for(Map m : res) {
			Backendless.Persistence.of( "foodOffers" ).remove(m);
		}
	}
